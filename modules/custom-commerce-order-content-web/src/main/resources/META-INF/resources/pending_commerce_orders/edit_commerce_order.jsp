...
<%
	...
	String resourceURL = "\"" + commerceOrderContentDisplayContext.getExportCommerceOrderReportURL() + "\"";
	...
	List<CommerceAddress> shippingAddresses = commerceOrderContentDisplayContext.getShippingCommerceAddresses(
			commerceAccount.getCommerceAccountId(), commerceAccount.getCompanyId());
	List<CommerceAddress> billingAddresses = commerceOrderContentDisplayContext.getBillingCommerceAddresses(
			commerceAccount.getCommerceAccountId(), commerceAccount.getCompanyId());

	List<String> errorMessages = (List<String>) request
			.getAttribute(CommerceWebKeys.COMMERCE_ORDER_ERROR_MESSAGES);
%>
...
<portlet:actionURL
	name="/commerce_open_order_content/edit_commerce_order"
	var="editCommerceOrderActionURL">
	<portlet:param name="mvcRenderCommandName"
		value="/commerce_open_order_content/edit_commerce_order" />
</portlet:actionURL>

<aui:form action="<%=editCommerceOrderActionURL%>"
	cssClass="order-details-container" method="post" name="fm">
	...
	<liferay-ui:error embed="<%=false%>"
		exception="<%=CommerceOrderImporterTypeException.class%>"
	...
	<liferay-ui:error embed="<%=false%>"
		...
		<%
			String commerceOrderImporterTypeKey = (String) SessionErrors.get(renderRequest,
							CommerceOrderImporterTypeException.class);
		%>
		...
		<c:choose>
			...
			<c:otherwise>
				<liferay-ui:message arguments="<%=commerceOrderImporterTypeKey%>"
					key="the-x-could-not-be-imported" />
			</c:otherwise>
		</c:choose>
	</liferay-ui:error>
		...
		<%
			List<CommerceOrderValidatorResult> commerceOrderValidatorResults = new ArrayList<>();
					...
					if (commerceOrderValidatorException != null) {
						commerceOrderValidatorResults = commerceOrderValidatorException
	...
	<liferay-ui:error embed="<%=false%>" key="notImportedRowsCount">
		...
		<c:choose>
			<c:when test="<%=notImportedRowsCount > 1%>">
				<liferay-ui:message arguments="<%=notImportedRowsCount%>"
					key="x-rows-were-not-imported" translateArguments="<%=false%>" />
			</c:when>
			<c:otherwise>
	...
	<liferay-ui:success key="importedRowsCount">
		...
		<c:choose>
			<c:when test="<%=importedRowsCount > 1%>">
				<liferay-ui:message arguments="<%=importedRowsCount%>"
					key="x-rows-were-imported-successfully"
	...
	<aui:model-context bean="<%=commerceOrder%>"
						...
						<dd>
							...
							<%
								request.setAttribute("order_notes.jsp-showLabel", Boolean.TRUE);
									request.setAttribute("order_notes.jsp-taglibLinkCssClass",
											"link-outline link-outline-borderless link-outline-secondary lfr-icon-item-reverse");
							%>

							<liferay-util:include
								page="/pending_commerce_orders/order_notes.jsp"
		...
		<div class="commerce-panel__content">
			<div class="align-items-center row">
				<div class="col-md-3">
					<dl class="commerce-list">
						<dt>
							<liferay-ui:message key="account-id" />
						</dt>
				...
				<div class="col-md-3">
					<dl class="commerce-list">
						<dt>
							<liferay-ui:message key="order-type" />
						</dt>
	...
	<c:if
		test="<%=commerceOrderContentDisplayContext.isShowPurchaseOrderNumber()%>">
		<div class="row">
			<div class="col-md-12">
				<div class="commerce-panel">
						...
						<div class="row">
							<div class="col-md-6">
								<dl class="commerce-list">
									<c:choose>
										<c:when
											test="<%=commerceOrderContentDisplayContext.hasModelPermission(commerceOrder,
									ActionKeys.UPDATE)%>">
											<aui:input cssClass="commerce-input" inlineField="<%=true%>"
												label="" name="purchaseOrderNumber"
												wrappedField="<%=false%>" />
										</c:when>
										<c:otherwise>
			...
			<div class="commerce-panel">
				...
				<div class="commerce-panel__content">
					<div class="row">
						<div class="col-md-12">
							<c:if
								test="<%=commerceOrderContentDisplayContext.hasViewBillingAddressPermission(permissionChecker,
							commerceAccount)%>">
								<c:choose>
									<c:when
										test="<%=commerceOrderContentDisplayContext.hasModelPermission(commerceOrder,
									ActionKeys.UPDATE)%>">
										<dl class="commerce-list">
											<aui:select cssClass="commerce-input" inlineField="<%=true%>"
												label="" name="billingAddressId" showEmptyOption="<%=true%>"
												wrappedField="<%=false%>">
												...
												<aui:option
													label="<%=HtmlUtil.escape(commerceAddress.getName())%>"
													selected="<%=billingCommerceAddressId == commerceAddress.getCommerceAddressId()%>"
									...
									<c:otherwise>
										<c:if test="<%=billingCommerceAddress != null%>">
											...
											<c:if
												test="<%=!Validator.isBlank(billingCommerceAddress.getStreet2())%>">
												<p><%=HtmlUtil.escape(billingCommerceAddress.getStreet2())%></p>
											</c:if>

											<c:if
												test="<%=!Validator.isBlank(billingCommerceAddress.getStreet3())%>">
												<p><%=HtmlUtil.escape(billingCommerceAddress.getStreet3())%></p>
											</c:if>

											<p><%=HtmlUtil.escape(billingCommerceAddress.getCity() + StringPool.SPACE
										+ billingCommerceAddress.getZip())%></p>
										</c:if>
									</c:otherwise>
								</c:choose>
							</c:if>
						...
						<div class="col-md-12">
							<c:choose>
								...
								ActionKeys.UPDATE)%>">
									<dl class="commerce-list">
										<aui:select cssClass="commerce-input" inlineField="<%=true%>"
											label="" name="shippingAddressId" showEmptyOption="<%=true%>"
											wrappedField="<%=false%>">
											...
											<aui:option
												label="<%=HtmlUtil.escape(commerceAddress.getName())%>"
												selected="<%=shippingCommerceAddressId == commerceAddress.getCommerceAddressId()%>"
												value="<%=commerceAddress.getCommerceAddressId()%>" />
								...
								<c:otherwise>
									<c:if test="<%=shippingCommerceAddress != null%>">
										...
										<c:if
											test="<%=!Validator.isBlank(shippingCommerceAddress.getStreet2())%>">
											<p><%=HtmlUtil.escape(shippingCommerceAddress.getStreet2())%></p>
										</c:if>

										<c:if
											test="<%=!Validator.isBlank(shippingCommerceAddress.getStreet3())%>">
											<p><%=HtmlUtil.escape(shippingCommerceAddress.getStreet3())%></p>
										</c:if>

										<p><%=HtmlUtil.escape(shippingCommerceAddress.getCity() + StringPool.SPACE
									+ shippingCommerceAddress.getZip())%></p>
									</c:if>
								</c:otherwise>
							</c:choose>
						</div>
	...
	<liferay-ui:icon-menu direction="right" icon="<%=StringPool.BLANK%>"
		...
		<%
			PortletURL viewCommerceOrderImporterTypeURL = PortletURLBuilder.createRenderURL(renderResponse)
							.setMVCRenderCommandName("/commerce_open_order_content/view_commerce_order_importer_type")
							.setParameter("commerceOrderId", commerceOrder.getCommerceOrderId())
							.setWindowState(LiferayWindowState.POP_UP).buildPortletURL();

					for (CommerceOrderImporterType commerceOrderImporterType : commerceOrderContentDisplayContext
						...
						viewCommerceOrderImporterTypeURL.setParameter("commerceOrderImporterTypeKey",
								commerceOrderImporterTypeKey);

						String viewCommerceOrderImporterTypeURLString = viewCommerceOrderImporterTypeURL.toString();
		%>

		<liferay-ui:icon cssClass="<%=commerceOrderImporterTypeKey%>"
			message="<%=commerceOrderImporterTypeLabel%>"
			url="<%=viewCommerceOrderImporterTypeURLString%>" />

		<liferay-frontend:component
			context='<%=HashMapBuilder
								.<String, Object>put("commerceOrderImporterTypeKey", commerceOrderImporterTypeKey)
								.put("title", commerceOrderImporterTypeLabel)
								.put("url", viewCommerceOrderImporterTypeURLString).build()%>'
		...
		<c:if
			...
			<portlet:actionURL
				...
				<portlet:param name="commerceOrderId"
					value="<%=String.valueOf(commerceOrder.getCommerceOrderId())%>" />
			</portlet:actionURL>
			...
			<portlet:actionURL
				name="/commerce_open_order_content/edit_commerce_order_item"
				var="deleteOrderContentURL">
				<portlet:param name="<%=Constants.CMD%>"
			...
			<liferay-ui:icon-delete message="remove-all-items"
				url="<%=deleteOrderContentURL%>" />

			<liferay-ui:icon icon="print" message="print" target="_blank"
				url="<%=commerceOrderContentDisplayContext.getExportCommerceOrderReportURL()%>" />
		</c:if>
	</liferay-ui:icon-menu>

	<div class="commerce-cta is-visible">
<!-- 		Action URL for archiving the order PDF in the document library -->
		<style>
			.btn.btn-create-pdf > .lexicon-icon {
				margin-left: 5px;
			}
		</style>
<%--
		<portlet:actionURL
			name="/commerce_order_content_custom/archive_pdf"
			var="commerceOrderArchivePDF">
		...
		<a class="btn btn-secondary btn-create-pdf mr-3" href="<%=commerceOrderArchivePDF%>">
			Create PDF
			<clay:icon symbol="document-pdf" />
		</a>
--%>
		<portlet:renderURL var="viewCreatePDFURL" windowState="<%= LiferayWindowState.POP_UP.toString() %>">
			<portlet:param name="mvcRenderCommandName" value="/commerce_order_content_custom/create_pdf" />
			<portlet:param name="commerceOrderId"
						   value="<%=String.valueOf(commerceOrder.getCommerceOrderId())%>" />
			<portlet:param name="accountId"
						   value="<%=String.valueOf(commerceAccount.getCommerceAccountId())%>" />
		...
		<aui:script require="commerce-frontend-js/utilities/eventsDefinitions as events">
			document.querySelector('#create_pdf').addEventListener('click', (e) => {
				e.preventDefault();
				...
				Liferay.Util.openWindow({
					...
					title: "PDF creating ...",
					uri: "<%= viewCreatePDFURL %>",
				});
			});
		</aui:script>

		<a class="btn btn-secondary btn-create-pdf mr-3" href="#;" id="create_pdf">
			Create PDF
			<clay:icon symbol="document-pdf" />
		</a>

		<portlet:renderURL var="viewCommerceOrderQuoteHistoryURL" windowState="<%= LiferayWindowState.POP_UP.toString() %>">
			<portlet:param name="mvcRenderCommandName" value="/commerce_order_content_custom/quote_history" />
			<portlet:param name="commerceOrderId"
						   value="<%=String.valueOf(commerceOrder.getCommerceOrderId())%>" />
			<portlet:param name="accountId"
						   value="<%=String.valueOf(commerceAccount.getCommerceAccountId())%>" />
			<portlet:param name="delta" value="10" />
		</portlet:renderURL>

		<aui:script require="commerce-frontend-js/utilities/eventsDefinitions as events">
			document.querySelector('#quote_history').addEventListener('click', (e) => {
				e.preventDefault();
				...
				Liferay.Util.openWindow({
					...
					title: "Quote History",
					uri: "<%= viewCommerceOrderQuoteHistoryURL %>",
				});
			});
		</aui:script>

		<a class="btn btn-secondary mr-3" href="#;" id="quote_history">
			Quote History
		</a>

		<c:if
			test="<%=commerceOrderContentDisplayContext.hasModelPermission(commerceOrder, ActionKeys.UPDATE)%>">
			<clay:button cssClass="btn-fixed btn-secondary"
				displayType="secondary"
				label='<%=LanguageUtil.get(request, "save")%>' small="<%=false%>"
				type="submit" />
		</c:if>
...
<div class="row">
	<div class="col-md-9">
		...
		<clay:data-set-display contextParams="<%=contextParams%>"
			...
			nestedItemsKey="orderItemId" nestedItemsReferenceKey="orderItems"
			pageNumber="<%=1%>"
			...
			<div class="commerce-panel__content">
				<dl class="commerce-list">
					...
					<c:if
						...
						&& (BigDecimal.ZERO.compareTo(subtotalCommerceDiscountValue.getDiscountPercentage()) < 0)%>">
						...
						<dt></dt>
						<dd class="text-right"><%=HtmlUtil.escape(commerceOrderContentDisplayContext
						.getLocalizedPercentage(subtotalCommerceDiscountValue.getDiscountPercentage(), locale))%></dd>
					</c:if>
					...
					<c:if
						test="<%=(shippingCommerceDiscountValue != null)
						&& (BigDecimal.ZERO.compareTo(shippingCommerceDiscountValue.getDiscountPercentage()) < 0)%>">
						...
						<dt></dt>
						<dd class="text-right"><%=HtmlUtil.escape(commerceOrderContentDisplayContext
						.getLocalizedPercentage(shippingCommerceDiscountValue.getDiscountPercentage(), locale))%></dd>
					</c:if>

					<c:if
						test="<%=priceDisplayType.equals(CommercePricingConstants.TAX_EXCLUDED_FROM_PRICE)%>">
						<dt>
					...
					<c:if
						...
						&& (BigDecimal.ZERO.compareTo(totalCommerceDiscountValue.getDiscountPercentage()) < 0)%>">
						...
						<dt>
							<liferay-ui:message key="total-discount" />
						</dt>
						<dd class="text-right"><%=HtmlUtil.escape(totalDiscountAmountCommerceMoney.format(locale))%></dd>
						<dt></dt>
						<dd class="text-right"><%=HtmlUtil.escape(commerceOrderContentDisplayContext
						.getLocalizedPercentage(totalCommerceDiscountValue.getDiscountPercentage(), locale))%></dd>
					</c:if>
				</dl>
			</div>
...
<liferay-ui:panel-container extended="<%=false%>"
	id="orderCollaborationPanelContainer" persistState="<%=true%>">
	<liferay-ui:panel collapsible="<%=false%>" extended="<%=true%>"
		id="orderCollaborationPanel" persistState="<%=true%>" title="">
		<liferay-comment:discussion
			className="<%=CommerceOrder.class.getName()%>"
			classPK="<%=commerceOrder.getCommerceOrderId()%>" formName="fm2"
